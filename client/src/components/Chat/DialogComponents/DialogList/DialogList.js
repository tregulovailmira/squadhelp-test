...
const DialogList = (props) => {
  ...
  const getTimeStr = (time) => {
    ...
    if (currentTime.isSame(time, 'day')) { return moment(time).format('HH:mm'); } else if (currentTime.isSame(time, 'week')) { return moment(time).format('dddd'); } else if (currentTime.isSame(time, 'year')) { return moment(time).format('MM DD'); } else { return moment(time).format('MMMM DD, YYYY'); }
  };
  ...
  const renderChatPreview = () => {
    ...
    if (chatMode === CONSTANTS.FAVORITE_PREVIEW_CHAT_MODE) { return renderPreview(onlyFavoriteDialogs); } else if (chatMode === CONSTANTS.BLOCKED_PREVIEW_CHAT_MODE) { return renderPreview(onlyBlockDialogs); } else { return renderPreview(); }
  };
